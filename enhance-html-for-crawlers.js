...
function verifyEnhancedFiles() {
  ...
  // Simple check function that doesn't require cheerio
  const checkFileForContent = (filePath, contentId) => {
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf8');
      return content.includes(contentId);
    }
    return false;
  };
  ...
  if (checkFileForContent(blogIndexPath, 'id="crawler-content"')) {
    enhancedCount++;
  } else if (fs.existsSync(blogIndexPath)) {
  ...
  if (checkFileForContent(faqPath, 'id="crawler-content"')) {
    enhancedCount++;
  ...
  if (fs.existsSync(blogDir)) {
    ...
    if (slugDirs.length > 0) {
      ...
      if (checkFileForContent(firstPostPath, 'id="crawler-content"')) {
        enhancedCount++;
      } else if (fs.existsSync(firstPostPath)) {
